...
def converted_laps(df):
    ...
    mdf1 = mdf1.apply(lambda x : (float(x.split(":")[0])* 60 + float(x.split(":")[1])))
...
def TrackPerformance(df):
    ...
    adf1 = adf1[adf1["Points"] > 15]["Team"].unique()
    adf2 = adf[adf["Position"] != "NC"]
    adf2 = adf2[adf2["Team"].isin(adf1)]

    adf2 = adf2.groupby("Engine Manufacturer").agg({"Points":"mean"})
    adf2 = adf2.sort_values(by = "Points",ascending = False)
